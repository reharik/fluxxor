...
Store.prototype.bindActions2 = function(actions) {
    if(actions.servicesActions){
        actions.map(function(action){
            createServicesActions(action);
        })
    }
    var createServicesActions = function(type){
        if(this.__actions__[type]){
            return;
        }
        if(!this['on'+type+'Success']){
            throw new Error('A Success Handler for action type ' + action.type + ' (on'+type+'Success) is not a defined');
        }
        this.__actions__[type] = function () {
            this.loading = true;
            this.emit("change");
        };
        this.__actions__[type+'Fail'] = function (payload) {
            this.loading = false;
            this.error = payload;
            this.emit("change");
        },
            this.__actions__[type+'Success'] = this['on'+type+'Success']()
    }
}
